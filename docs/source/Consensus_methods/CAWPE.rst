...
The output of this pipeline consists of a vector W of size m (#methods), where Wm represents the performance of method m.

2. Individual method training and cell class confidence prediction
------------
The Individual method performance assessment is performed by running the :ref:`config_annotation` pipeline.
As a result, for each cell k we obtain a probability matrix Pk of size m x c (methods by cells), where Pkij represents the probability of cell k belonging to class i as determined by the method j.

3. Consensus calculation
------------
The ensemble mapping score is calculated by combining the individual methods predictions (Pk) and their performance scores (W). 
